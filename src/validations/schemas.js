...
export const verifyValidation = Joi.object({
  ...
  email: Joi.string().email().pattern(new RegExp(/^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/)).when("verificationMethod", {
    is: "email",
    then: Joi.required().messages({
      "string.email": "Please enter a valid email",
      "string.empty": "Email is required when verification method is email"
    }),
    otherwise: Joi.forbidden()
  }),
  phone: Joi.string().pattern(/^(\+92|0)?3\d{9}$/).when("verificationMethod", {
    is: "phone",
    then: Joi.required().messages({
      "string.pattern.base": "Phone must be valid number (11 digits)",
      "string.empty": "Phone is required when verification method is phone"
    }),
    otherwise: Joi.forbidden()
  }),
  verificationCode: Joi.number().integer().min(100000).max(999999).required().messages({
    ...
    "number.min": "Verification code must be a 6-digit number",
    "number.max": "Verification code must be a 6-digit number",
